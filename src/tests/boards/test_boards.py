...
from djanban.apps.boards.forms import EditBoardForm, NewBoardForm
...
class BoardTest(TestCase):
    ...
    def test_create_board(self):
        ...
        self.assertEqual(
            ...
            "Board descriptions do not match",
        )
        ...
        self.assertTrue(
            new_board.members.filter(id=self.member.id).exists(),
            "Member creator is not a member of the board",
        )
        # Testing if creator is admin
        member_role_for_new_board = MemberRole.objects.get(
            board=new_board, type="admin"
        )
        self.assertTrue(
            member_role_for_new_board.members.filter(id=self.member.id).exists(),
    ...
    def test_edit_board(self):
        ...
        for field, value in list(form_data.items()):
            if field != "hourly_rates" and field != "header_image":
                self.assertEqual(
                    getattr(edited_board, field),
                    value,
                    "Attribute {0} does not match".format(field),
                )
