...
class GitRepository(object):
    ...
    def _checkout(self, commit=None):
        ...
        if commit:
            checkout_command = "cd {0} && git checkout {1} && cd -".format(
                repository_dir, commit
            )
...
class GitHubPublicRepository(Repository, GitRepository):
    ...
    @property
    def repository_path(self):
        return "{0}{1}/{2}".format(settings.TMP_DIR, self.username, self.name)

    @property
    def namespace_path(self):
        return "{0}{1}".format(settings.TMP_DIR, self.username)

    @property
    def clone_command(self):
        repository_dir = self.repository_path
        return "git clone https://github.com/{0}/{1}.git {2}".format(
            self.username, self.name, repository_dir
        )
    ...
    def fetch_commit_info(self, commit):
        # Get commit info
        gh = Github()
        repo = gh.get_user(self.username).get_repo(self.name)
...
class GitLabRepository(Repository, GitRepository):
    ...
    def __str__(self):
        return "Access token: {0}..., Username: {1} and project name: {2}".format(
            self.access_token[0:5], self.username, self.project_name
        )

    @property
    def project_full_name(self):
        return "{0}/{1}".format(self.project_userspace, self.project_name)

    @property
    def repository_path(self):
        return "{0}{1}/{2}".format(
            settings.TMP_DIR, self.project_userspace, self.project_name
        )

    @property
    def namespace_path(self):
        return "{0}{1}".format(settings.TMP_DIR, self.project_userspace)
