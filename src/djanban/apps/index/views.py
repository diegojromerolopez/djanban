...
def index(request):
    ...
    if user_is_member(current_user):
        ...
        member_multiboards = member.multiboards.filter(
            show_in_index=True, is_archived=False
        ).order_by("order", "name")
    ...
    if current_user.is_authenticated():
        ...
        boards = (
            get_user_boards(current_user).filter(is_archived=False).order_by("name")
        )
    ...
    replacements = {
        "any_card_has_value": Card.objects.filter(
            board__in=boards, value__isnull=False
        ).exists(),
        "has_noise_measurements": NoiseMeasurement.objects.filter(
            Q(member__in=team_mates) | Q(member=member)
        ).exists(),
        ...
        "developers": [member]
        + (
            list(member.team_mates.filter(is_developer=True))
            if member
            else list(team_mates)
        ),
        "downtime_developers": (
            (
                [
                    dev
                    for dev in member.team_members.filter(is_developer=True)
        ...
        "pending_red_cards": Card.objects.filter(
            board__in=boards,
            list__type="ready_to_develop",
            is_closed=False,
            labels__color="red",
        ).order_by("board__name", "name"),
        "pending_orange_cards": Card.objects.filter(
            board__in=boards,
            list__type="ready_to_develop",
            is_closed=False,
            labels__color="orange",
        ).order_by("board__name", "name"),
        "pending_yellow_cards": Card.objects.filter(
            board__in=boards,
            list__type="ready_to_develop",
            is_closed=False,
