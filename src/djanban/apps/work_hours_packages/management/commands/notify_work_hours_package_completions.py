...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for package in candidate_to_completion_packages:
            ...
            if percentage:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"Notifications for {percentage} of {package.full_name} sent successfully")
                )
    ...
    def send_package_notification(self, package, percentage):
        ...
        try:
            ...
            message.attach(f'spent_times-for-{package.name}.csv', csv_report, 'text/csv')
            message.send()
            # Mark this work hours package as notified
            package.mark_completion_notification_as_sent(percentage)
        except Exception:
            self.stdout.write(
                self.style.ERROR(f"Notifications of {package.name} could not be sent successfully")
            )
