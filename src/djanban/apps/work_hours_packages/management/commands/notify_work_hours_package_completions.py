...
class Command(BaseCommand):
    help = "Notify if any work hours package has been completed"
    ...
    def handle(self, *args, **options):
        ...
        if self.member:
            candidate_to_completion_packages = candidate_to_completion_packages.filter(
                members__in=self.member
            )
        ...
        for package in candidate_to_completion_packages:
            ...
            if (
                package.completion_notification_datetime is None
                and adjusted_time >= package.number_of_hours
            ):
                percentage = "100%"
            elif (
                package.ninety_percent_completion_notification_datetime is None
                and adjusted_time >= package.number_of_hours * Decimal(0.9)
            ):
                percentage = "90%"
            elif (
                package.eighty_percent_completion_notification_datetime is None
                and adjusted_time >= package.number_of_hours * Decimal(0.8)
            ):
                percentage = "80%"
            elif (
                package.half_completion_notification_datetime is None
                and adjusted_time >= package.number_of_hours * Decimal(0.5)
            ):
                percentage = "50%"
            ...
            if percentage:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Notifications for {percentage} of {package.full_name} sent successfully"
                    )
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"{number_of_sent_notifications} notification(s) sent successfully"
                )
            )
    ...
    def send_package_notification(self, package, percentage):
        ...
        emails = {
            ...
            for member in package.members.exclude(user__email="")
        }
        ...
        txt_message = get_template(
            "work_hours_packages/emails/completion_notification.txt"
        ).render(replacements)
        html_message = get_template(
            "work_hours_packages/emails/completion_notification.html"
        ).render(replacements)

        subject = f"[Djanban][WorkHoursPackage] Work hours package {package.name} at {percentage}"
        ...
        try:
            ...
            message = EmailMultiAlternatives(
                subject, txt_message, settings.EMAIL_HOST_USER, bcc=emails
            )
            message.attach_alternative(html_message, "text/html")
            message.attach(
                f"spent_times-for-{package.name}.csv", csv_report, "text/csv"
            )
            message.send()
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"Notifications of {package.name} could not be sent successfully"
                )
            )
