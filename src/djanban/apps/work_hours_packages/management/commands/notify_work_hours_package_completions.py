...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for package in candidate_to_completion_packages:
            ...
            if percentage:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        "Notifications for {0} of {1} sent successfully".format(
                            percentage, package.full_name
                        )
                    )
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "{0} notification(s) sent successfully".format(
                        number_of_sent_notifications
                    )
                )
            )
    ...
    def send_package_notification(self, package, percentage):
        ...
        subject = "[Djanban][WorkHoursPackage] Work hours package {0} at {1}".format(
        ...
        try:
            ...
            message.attach(
                "spent_times-for-{0}.csv".format(package.name), csv_report, "text/csv"
            )
            message.send()
            ...
            self.stdout.write(
                self.style.ERROR(
                    "Notifications of {0} could not be sent successfully".format(
                        package.name
                    )
                )
            )
