...
from djanban.apps.work_hours_packages.management.commands.notify_work_hours_package_completions import (
    Command,
)
...
class WorkHoursPackageFilterForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        boards = (
            get_member_boards(self.member).filter(is_archived=False).order_by("name")
        )
    ...
    def get_work_hours_packages(self):
        ...
        if (
            self.cleaned_data.get("is_paid") == "Yes"
            or self.cleaned_data.get("is_paid") == "No"
        ):
            work_hours_packages = work_hours_packages.filter(
...
class WorkHoursPackageForm(forms.ModelForm):
    ...
    class Media:
        css = {"all": ("css/work_hours_packages/form.css",)}
        js = ("js/work_hours_packages/form.js",)
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Available boards for this user
        boards = (
            get_member_boards(current_member).filter(is_archived=False).order_by("name")
        )
        self.fields["board"].choices = [("", "None")] + [
            (board.id, board.name) for board in boards
        ]
    ...
    def clean(self):
        ...
        if (
            (
                not cleaned_data.get("board")
                and not cleaned_data.get("label")
                and not cleaned_data.get("multiboard")
            )
            or (
                cleaned_data.get("board")
                and cleaned_data.get("label")
                and cleaned_data.get("multiboard")
            )
            or (cleaned_data.get("board") and cleaned_data.get("label"))
            or (cleaned_data.get("board") and cleaned_data.get("multiboard"))
            or (cleaned_data.get("label") and cleaned_data.get("multiboard"))
        ):
        ...
        if cleaned_data.get("type") == "multiboard" and not cleaned_data.get(
            "multiboard"
        ):
            raise ValidationError(
                "Please, select a multiboard for this work hours package"
            )
...
class NotificationCompletionSenderForm(forms.Form):
    confirmed = forms.BooleanField(
        label="Confirm you want to send completion notifications"
    )
...
class DeleteWorkHoursPackageForm(forms.Form):
    confirmed = forms.BooleanField(
        label="Confirm you want to delete this work hours package"
    )
