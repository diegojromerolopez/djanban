...
from djanban.apps.password_reseter.forms import (
...
def request_password_reset(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "password_reseter/request_password_reset.html",
                {"requester_user": user},
            )
    else:
    ...
    return render(
        request, "password_reseter/request_password_reset.html", {"form": form}
    )
...
def reset_password(request, uuid):
    ...
    password_request = get_object_or_404(
        PasswordResetRequest, uuid=uuid, status="pending", limit_datetime__gt=now
    )
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "password_reseter/reset_password.html",
                {"requester_user": password_request.user},
            )
    else:
