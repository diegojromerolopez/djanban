...
def send_password_request_link(password_request, user):
    ...
    replacements = {
        "user": user,
        "absolute_reset_password_url": absolute_reset_password_url,
    }

    txt_message = get_template(
        "password_reseter/emails/request_password_reset.txt"
    ).render(replacements)
    html_message = get_template(
        "password_reseter/emails/request_password_reset.html"
    ).render(replacements)
...
def send_password_reset_successfully_email(user):
    ...
    txt_message = get_template(
        "password_reseter/emails/password_reset_successfully.txt"
    ).render(replacements)
    html_message = get_template(
        "password_reseter/emails/password_reset_successfully.html"
    ).render(replacements)
