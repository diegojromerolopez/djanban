...
class RecurrentCard(models.Model):
    ...
    creator = models.ForeignKey(
        "members.Member",
        related_name="created_recurrent_cards",
        verbose_name="Created recurrent cards",
        on_delete=models.CASCADE,
    )
    board = models.ForeignKey(
        "boards.Board",
        related_name="recurrent_cards",
        verbose_name="Recurrent cards",
        on_delete=models.CASCADE,
    )

    name = models.CharField(verbose_name="Recurrent card name", max_length=512)
    description = models.TextField(
        verbose_name="Description of the card", default="", blank=True
    )
    position = models.CharField(
        verbose_name="Position in the list",
    ...
    creation_list = models.ForeignKey(
        "boards.List",
        related_name="recurrent_cards",
    ...
    labels = models.ManyToManyField(
        "boards.Label", related_name="recurrent_cards", blank=True
    )
    ...
    def create_card(self):
        ...
        if self.estimated_time:
            new_card.add_spent_estimated_time(
                member=self.creator, spent_time=0, estimated_time=self.estimated_time
            )
...
class WeeklyRecurrentCard(RecurrentCard):
    create_on_mondays = models.BooleanField(
        verbose_name="Create card on mondays", default=False
    )
    create_on_tuesdays = models.BooleanField(
        verbose_name="Create card on tuesdays", default=False
    )
    create_on_wednesdays = models.BooleanField(
        verbose_name="Create card on wednesdays", default=False
    )
    create_on_thursdays = models.BooleanField(
        verbose_name="Create card on thursdays", default=False
    )
    create_on_fridays = models.BooleanField(
        verbose_name="Create card on fridays", default=False
    )
    create_on_saturdays = models.BooleanField(
        verbose_name="Create card on saturdays", default=False
    )
    create_on_sundays = models.BooleanField(
        verbose_name="Create card on sundays", default=False
    )

    move_to_list_when_day_ends = models.ForeignKey(
        "boards.List",
        verbose_name="Automatically move the card to this list when the day ends",
        related_name="moved_recurrent_cards",
        default=None,
        null=True,
        blank=True,
        on_delete=models.CASCADE,
