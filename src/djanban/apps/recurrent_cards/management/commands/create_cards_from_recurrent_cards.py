...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for recurrent_card in recurrent_cards:
                ...
                if not has_created_a_card_today:
                    ...
                    num_created_cards += 1
                    self.stdout.write(
                        self.style.SUCCESS("{0} successfully created".format(card.name))
                    )
                # In case a card has been already created for this recurrent card, show a warning
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            "card {0} already created today".format(recurrent_card.name)
                        )
                    )
        ...
        if num_created_cards > 0:
            self.stdout.write(
                self.style.SUCCESS(
                    "Creation of {0} card(s) from recurrent cards completed successfully".format(
                        num_created_cards
                    )
                )
            )
