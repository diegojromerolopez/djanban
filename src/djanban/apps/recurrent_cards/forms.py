...
class RecurrentCardFilterForm(forms.Form):
    ...
    def get_recurrent_cards(self):
        ...
        recurrent_cards = self.member.recurrent_cards.filter(board=self.board).order_by(
            "name"
        )
        if label_id:
        ...
        if (
            self.cleaned_data.get("is_active") == "Yes"
            or self.cleaned_data.get("is_active") == "No"
        ):
            recurrent_cards = recurrent_cards.filter(
                is_paid=(self.cleaned_data.get("is_active") == "Yes")
            )
...
class WeeklyRecurrentCardForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            ...
            "create_on_mondays",
            "create_on_tuesdays",
            "create_on_wednesdays",
            "create_on_thursdays",
            "create_on_fridays",
            "create_on_saturdays",
            "create_on_sundays",
            "move_to_list_when_day_ends",
            "is_active",
        ]

    class Media:
        css = {"all": ("css/recurrent_cards/form.css",)}
        js = ("js/recurrent_cards/form.js",)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["move_to_list_when_day_ends"].choices = [
            ("", "Don't move")
        ] + active_lists
...
class DeleteWeeklyRecurrentCardForm(forms.Form):
    confirmed = forms.BooleanField(
        label="Confirm you want to delete this recurrent card"
    )
