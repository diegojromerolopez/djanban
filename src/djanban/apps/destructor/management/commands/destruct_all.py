...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if (
            not hasattr(settings, "DESTRUCTION_PASSWORD_CHECKSUM")
            or settings.DESTRUCTION_PASSWORD_CHECKSUM is None
        ):
            self.stdout.write(
                self.style.ERROR("This installation does not allow destruction")
            )
            return False
        ...
        if (
            ...
            != settings.DESTRUCTION_PASSWORD_CHECKSUM
        ):
            self.stdout.write(
                self.style.ERROR("Password is not correct, aborting destruction")
            )
            return False

        self.stdout.write(
            self.style.SUCCESS("Password is correct, starting destruction")
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Database and media files have been destructed successfully"
            )
        )
