    ...
    def add_card_comment(card_comment, card):
        ...
        if card_comment.review:
            for member in card.members.all():
                Notification(
                    ...
                    sender=card_comment.author,
                    receiver=member,
                    description="{0}: review of card {1} by {2}".format(
                        board.name, card.name, card_comment.author
                    ),
                ).save()
        ...
        for member in members:
            Notification(
                ...
                sender=card_comment.author,
                receiver=member,
                description="{0}: Mention of {1} in comment {2}".format(
                    board.name, member.external_username, card.name
                ),
            ).save()
    ...
    def move_card(mover, card, board=None):
        ...
        for member in card.members.all():
            Notification(
                ...
                sender=mover,
                receiver=member,
                description="{0}: card {1} moved to {2}".format(
                    board.name, card.name, card.list.name
                ),
            ).save()
            ...
            for blocked_card in blocked_cards:
                ...
                for member in card.members.all():
                    Notification(
                        ...
                        sender=mover,
                        receiver=member,
                        description="{0}: card {1} is no longer blocked by {2}".format(
                            board.name, blocked_card.name, card.name
                        ),
                    ).save()
                    ...
                    if not blocked_card.blocking_cards.exclude(
                        ...
                        Notification(
                            ...
                            sender=mover,
                            receiver=member,
                            description="{0}: card {1} can be started".format(
                                board.name, blocked_card.name
                            ),
                        ).save()
