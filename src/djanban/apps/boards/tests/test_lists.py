...
class ListTest(TestCase):
    ...
    def setUp(self):
        ...
        for list_type in List.ACTIVE_LIST_TYPES:
            ...
            form = NewListForm(
                data=form_data, instance=List(board=self.board), member=self.member
            )
    ...
    def test_move_list(self):
        ...
        for list_type in List.ACTIVE_LIST_TYPES:
            move_up_form = MoveUpListForm(
                data={"movement_type": "up"}, instance=new_list, member=self.member
            )
            self.assertTrue(move_up_form.is_valid())
            move_up_form.save()
        self.assertTrue(
            new_list.is_first,
            "List should be the first one (was moved up to the first position)",
        )

        # Move the new list down 6 times to be the last list
        for list_type in List.ACTIVE_LIST_TYPES:
            move_down_form = MoveDownListForm(
                data={"movement_type": "down"}, instance=new_list, member=self.member
            )
            self.assertTrue(move_down_form.is_valid())
            move_down_form.save()
        self.assertTrue(
            new_list.is_last,
            "List should be the last one (was moved down to the last position)",
        )
    ...
    def test_swap_list(self):
        ...
        new_list_form = NewListForm(
            data=new_list_form_data, instance=List(board=self.board), member=self.member
        )
        self.assertTrue(new_list_form.is_valid(), "Last list is not valid")
        new_list = new_list_form.save(commit=True)
        # Swap this new list with the first one
        swap_form_data = {"list_to_be_swapped_with": self.lists[0].id}
        swap_list_form = SwapListForm(
            data=swap_form_data, instance=new_list, member=self.member
        )
