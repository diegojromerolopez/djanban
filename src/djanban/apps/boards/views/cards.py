...
def view(request, board_id, card_id):
    return HttpResponseRedirect(
        reverse("boards:view_taskboard", args=(board_id, "/card/{0}".format(card_id)))
    )
...
def view_short_url(request, board_id, card_uuid):
    ...
    return HttpResponseRedirect(
        reverse("boards:view_taskboard", args=(board.id, "/card/{0}".format(card.id)))
    )
...
def export_report(request, board_id):
    ...
    response["Content-Disposition"] = 'attachment; filename="{0}-cards.csv"'.format(
        board.name
    )
...
def export_detailed_report(request, board_id):
    ...
    response["Content-Disposition"] = (
        'attachment; filename="{0}-detailed-card-report.csv"'.format(board.name)
    )
