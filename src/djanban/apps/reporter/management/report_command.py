...
class ReportCommand(BaseCommand):
    ...
    def send_reports(
        ...
        for report_recipient in report_recipients:
            ...
            self.stdout.write(
                self.style.SUCCESS("Report sent to {0}".format(report_recipient.email))
            )
    ...
    def send_report(
        ...
        replacements = {
            ...
            "developer_members": Member.objects.filter(
                is_developer=True, on_holidays=False
            ).order_by("trello_member_profile__username"),
        ...
        message = EmailMultiAlternatives(
            subject, txt_message, settings.EMAIL_HOST_USER, [report_recipient.email]
        )
