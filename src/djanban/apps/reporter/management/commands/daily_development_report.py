...
class Command(daily_report.Command):
    ...
    def handle(self, *args, **options):
        ...
        if options["date"]:
            try:
                ...
                self.stderr.write(
                    self.style.ERROR(
                        "Date {} format is not valid".format(options["date"])
                    )
                )
                return None
        ...
        try:
            for member in developers:
                if (
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Daily report sent to developer {member.user.email}"
                        )
                    )
                elif member.user:
                    self.stdout.write(
                        self.style.WARNING(
                            "Developer {} has not worked on day {}".format(
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"Developer {member.external_username} has no email"
                        )
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Daily development reports for day {} sent successfully to {} developers in {} s".format(
    ...
    def send_daily_development_report(date, developer_member, daily_spent_times):
        ...
        subject = "[Djanban][DevReports] Daily development report of {}".format(
            date.strftime("%Y-%m-%d")
        )
        ...
        message.attach(
            "spent_times-for-day-{}.csv".format(date.strftime("%Y-%m-%d")),
            csv_report,
            "text/csv",
        )
        message.send()
