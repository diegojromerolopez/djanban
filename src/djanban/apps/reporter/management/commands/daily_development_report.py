...
class Command(daily_report.Command):
    ...
    def handle(self, *args, **options):
        ...
        if options["date"]:
            try:
                ...
                self.stderr.write(
                    self.style.ERROR(
                        "Date {0} format is not valid".format(options["date"])
                    )
                )
                return None
        ...
        if iso_weekday == 6 or iso_weekday == 7:
            self.stdout.write(
                self.style.SUCCESS(
                    "Daily development reports for day {0} are not sent because that day is holiday".format(
                        date.strftime("%Y-%m-%d")
                    )
                )
            )
        ...
        try:
            for member in developers:
                if (
                    member.daily_spent_times.filter(date=date).count() > 0
                    and member.user
                ):
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            "Daily report sent to developer {0}".format(
                                member.user.email
                            )
                        )
                    )
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            "Developer {0} has not worked on day {1}".format(
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            "Developer {0} has no email".format(
            ...
            warn_administrators(
                subject="Error in Daily development report",
                message=traceback.format_exc(),
            )
            self.stdout.write(
                self.style.ERROR("Error in the daily development report ")
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Daily development reports for day {0} sent successfully to {1} developers in {2} s".format(
    ...
    def send_daily_development_report(date, developer_member, daily_spent_times):
        ...
        replacements = {
            ...
            "developer_daily_spent_times": daily_spent_times.filter(
                member=developer_member
            ),
        }

        txt_message = get_template(
            "reporter/emails/daily_development_report.txt"
        ).render(replacements)
        html_message = get_template(
            "reporter/emails/daily_development_report.html"
        ).render(replacements)

        subject = "[Djanban][DevReports] Daily development report of {0}".format(
            date.strftime("%Y-%m-%d")
        )
        ...
        message = EmailMultiAlternatives(
            ...
            [developer_member.user.email],
        )
        message.attach_alternative(html_message, "text/html")
        message.attach(
            "spent_times-for-day-{0}.csv".format(date.strftime("%Y-%m-%d")),
            csv_report,
            "text/csv",
        )
        message.send()
