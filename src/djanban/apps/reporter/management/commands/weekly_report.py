...
class Command(ReportCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Weekly reports sent to {report_recipient.count()} administrators"))

        end = time.time()
        elapsed_time = end - start

        self.stdout.write(
            self.style.SUCCESS("Weekly reports for week {}/W{} sent successfully to {} in {} s".format(
