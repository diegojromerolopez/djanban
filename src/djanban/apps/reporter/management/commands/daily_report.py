...
class Command(ReportCommand):
    ...
    def handle(self, *args, **options):
        ...
        if iso_weekday == 6 or iso_weekday == 7:
            self.stdout.write(
                self.style.SUCCESS("Daily reports for day {} are not sent because that day is holiday".format(
        ...
        subject = "[Djanban][Reports] Daily report of {}".format(self.date.strftime("%Y-%m-%d"))
        txt_template_path = 'reporter/emails/daily_report.txt'
        html_template_path = 'reporter/emails/daily_report.html'
        csv_file_name = 'spent_times-for-day-{}.csv'.format(self.date.strftime("%Y-%m-%d"))

        report_recipient = self.send_reports(daily_spent_times, subject,
                                                txt_template_path, html_template_path, csv_file_name)
        self.stdout.write(self.style.SUCCESS(f"Daily reports sent to {report_recipient.count()} administrators"))

        end = time.time()
        elapsed_time = end-start

        self.stdout.write(
            self.style.SUCCESS("Daily reports for day {} sent successfully to {} in {} s".format(
