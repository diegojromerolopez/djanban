...
class Command(ReportCommand):
    ...
    def handle(self, *args, **options):
        ...
        if iso_weekday == 6 or iso_weekday == 7:
            self.stdout.write(
                self.style.SUCCESS(
                    "Daily reports for day {0} are not sent because that day is holiday".format(
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Daily reports sent to {0} administrators".format(
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Daily reports for day {0} sent successfully to {1} in {2} s".format(
                    self.date.strftime("%Y-%m-%d"),
                    report_recipient.count(),
                    elapsed_time,
                )
