...
class Command(ReportCommand):
    ...
    def handle(self, *args, **options):
        ...
        if iso_weekday == 6 or iso_weekday == 7:
            self.stdout.write(
                self.style.SUCCESS(
                    "Daily reports for day {} are not sent because that day is holiday".format(
        ...
        daily_spent_times = DailySpentTime.objects.filter(date=self.date).order_by(
            "date", "member"
        )

        subject = "[Djanban][Reports] Daily report of {}".format(
            self.date.strftime("%Y-%m-%d")
        )
        txt_template_path = "reporter/emails/daily_report.txt"
        html_template_path = "reporter/emails/daily_report.html"
        csv_file_name = "spent_times-for-day-{}.csv".format(
            self.date.strftime("%Y-%m-%d")
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Daily reports sent to {report_recipient.count()} administrators"
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Daily reports for day {} sent successfully to {} in {} s".format(
                    self.date.strftime("%Y-%m-%d"),
