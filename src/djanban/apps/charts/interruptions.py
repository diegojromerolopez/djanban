...
def number_of_interruptions(current_user, board=None):
    chart_title = "Number of interruptions as of {0}".format(timezone.now())
...
def evolution_of_interruptions(current_user, board=None):
    chart_title = "Evolution of number of interruptions as of {0}".format(
        timezone.now()
    )
    return _number_of_interruptions(
...
def interruption_spent_time(current_user, board=None):
    chart_title = "Interruption spent time as of {0}".format(timezone.now())
    return _number_of_interruptions(
        current_user,
        board,
        chart_title,
...
def evolution_of_interruption_spent_time(current_user, board=None):
    chart_title = "Evolution of interruption spent time as of {0}".format(
        timezone.now()
    )
    return _number_of_interruptions(
...
def _number_of_interruptions(
    ...
    chart_uuid = "interruptions.{0}".format(
        hashlib.sha256(
            "_number_of_interruptions-{0}-{1}-{2}-{3}-{4}".format(
                current_user.id,
...
def number_of_interruptions_by_member(current_user):
    chart_title = "Number of interruptions by member as of {0}".format(timezone.now())
...
def evolution_of_interruptions_by_member(current_user):
    chart_title = "Evolution of number of interruptions by member as of {0}".format(
        timezone.now()
    )
    return _number_of_interruptions_by_member(
...
def interruption_spent_time_by_member(current_user):
    chart_title = "Spent time on interruptions by member as of {0}".format(
        timezone.now()
    )
    return _number_of_interruptions_by_member(
...
def _number_of_interruptions_by_member(
    ...
    chart_uuid = "interruptions.{0}".format(
        hashlib.sha256(
            "_number_of_interruptions_by_member-{0}-{1}-{2}".format(
                current_user.id,
                inspect.getsource(interruption_measurement),
                "incremental" if incremental else "absolute",
            )
...
def number_of_interruptions_by_month(current_user, board=None):
    chart_title = "Number of interruptions by month as of {0}".format(timezone.now())
...
def interruption_spent_time_by_month(current_user, board=None):
    chart_title = "Interruption spent time by month as of {0}".format(timezone.now())
    return _interruption_measurement_by_month(
...
def _interruption_measurement_by_month(
    ...
    chart_uuid = "interruptions.{0}".format(
        hashlib.sha256(
            "_interruption_measurement_by_month-{0}-{1}-{2}".format(
                current_user.id,
                inspect.getsource(interruption_measurement),
    ...
    while year_i < last_year or year_i == last_year and month_i <= last_month:
        ...
        if monthly_measurement > 0:
            months.append("{0}-{1}".format(year_i, month_i))
