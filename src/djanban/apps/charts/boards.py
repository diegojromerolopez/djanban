...
def burndown(board, show_interruptions=False):
    ...
    chart_uuid = "boards.burndown-{0}".format(
    ...
    chart_title = "Burndown for board {0}".format(board.name)
    ...
    daily_spent_times = (
        board.daily_spent_times.filter(spent_time__gt=0).distinct().order_by("date")
    )
    ...
    while date_i <= end_working_date:
        ...
        spent_time = daily_spent_times_i.aggregate(spent_time_sum=Sum("spent_time"))[
            "spent_time_sum"
        ]
    ...
    burndown_chart.add(
        "Initial estimation for {0}".format(board.name),
        [remaining_time for i in range(0, len(x_labels))],
    )

    burndown_chart.x_labels = x_labels
    burndown_chart.add("Burndown of {0}".format(board.name), remaining_time_values)

    if show_interruptions:
        burndown_chart.add("Interruptions of {0}".format(board.name), interruptions)
