...
def number_of_code_errors(grouped_by, board, repository, language="python"):
    ...
    raise ValueError("Value {0} not recognized".format(grouped_by))
...
def _number_of_code_errors_by_commit(
    ...
    if repository:
        ...
        repository_text = ", repository {0}, ".format(repository.name)

    if not per_loc:
        chart_title = "Errors in {0} code by commit in project {1}{2}{3}".format(
            language, board.name, repository_text, timezone.now()
        )
    else:
        chart_title = (
            "Errors in {0} code per LOC by commit in project {1}{2}{3}".format(
                language, board.name, repository_text, timezone.now()
            )
        )
    ...
    if language.lower() == "php":
        ...
        raise ValueError("Programming language {0} not recognized".format(language))
...
def _number_of_code_errors_by_month(
    ...
    if not per_loc:
        chart_title = "Errors in {0} code by month in project {1}{2}{3}".format(
            language, board.name, repository_text, timezone.now()
        )
    else:
        chart_title = "Errors in {0} code per LOC by month in project {1}{2}{3}".format(
            language, board.name, repository_text, timezone.now()
        )
    ...
    for message_type in message_types:
        ...
        while year_i < max_year_i or (year_i == max_year_i and month_i <= max_month_i):
            ...
            chart.x_labels.append("{0}-{1}".format(year_i, month_i))
