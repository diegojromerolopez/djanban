...
def _requirement_burndown(board, requirement):
    ...
    chart_title = "Burndown of requirement {0}".format(requirement.code)
    ...
    burndown_chart.add(
        "Initial estimation for {0}".format(requirement.code),
        [remaining_time for i in range(0, len(x_labels))],
    )
    burndown_chart.x_labels = x_labels
    burndown_chart.add(
        "Burndown of {0}".format(requirement.code), remaining_time_values
    )
...
def _burndown_by_requirement(board):
    ...
    burndown_chart.add(
        "{0} requirements estimation".format(board.name),
        [remaining_time for i in range(0, len(x_labels))],
    )
    burndown_chart.x_labels = x_labels
    burndown_chart.add(
        "Burndown according to {0} requirements".format(board.name),
