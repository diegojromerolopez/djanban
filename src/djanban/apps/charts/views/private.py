...
def spent_time_by_day_of_the_week(
    ...
    chart_title = "{0}'s spent time in week {1} ({2} - {3})".format(
    ...
    try:
        ...
        while day <= end_of_week:
            ...
            spent_time_chart.add("{0}".format(day.strftime("%A")), member_spent_time)
...
def time_scatterplot(request, time_metric, board_id=None, year=None, month=None):
    ...
    if time_metric == "lead_time":
        ...
        raise ValueError("Time metric {0} not recognized".format(time_metric))
...
def completion_histogram(
    ...
    if units is None:
        ...
        raise ValueError("Units value {0} not recognized".format(units))
