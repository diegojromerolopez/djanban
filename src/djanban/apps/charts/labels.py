...
def avg_spent_times(request, board=None):
    ...
    # Caching
    chart_uuid = "labels.avg_spent_times-{0}".format(
    ...
    chart_title = "Average task spent time as of {0}".format(timezone.now())
    if board:
    ...
    if board:
        ...
        for board in boards:
            ...
            if board_avg_spent_time > 0:
                avg_times_chart.add("{0}".format(board.name), board_avg_spent_time)
        ...
        for label in labels:
            if label.name:
                ...
                if label_avg_spent_time:
                    avg_times_chart.add(
                        "{0} - {1}".format(board.name, label.name), label_avg_spent_time
                    )
...
def avg_estimated_times(request, board=None):
    ...
    chart_title = "Average task estimated time as of {0}".format(timezone.now())
    if board:
        chart_title += " for board {0}".format(board.name)
    ...
    if board:
        ...
        for board in boards:
            ...
            if board_avg_estimated_time > 0:
                avg_times_chart.add("{0}".format(board.name), board_avg_estimated_time)
        ...
        for label in labels:
            if label.name:
                ...
                if label_avg_estimated_time > 0:
                    avg_times_chart.add(
                        "{0} - {1}".format(board.name, label.name),
...
def _daily_spent_times_by_period(
    ...
    if operation == "Avg":
        ...
        chart_title = "Tasks worked on as of {0}".format(last_activity_datetime)
        if board:
            chart_title += " for board {0} (fetched on {1})".format(
                board.name, board.get_human_fetch_datetime()
            )
    ...
    while not end_loop:
        if period == "month":
            ...
            measurement_title = "{0}-{1}".format(year_i, month_i)
            "{0}-{1}".format(year_i, month_i)
            end_loop = (
                datetime.datetime.strptime(
                    "{0}-{1}-1".format(year_i, month_i), "%Y-%m-%d"
                ).date()
                > end_date
            )
        elif period == "week":
            period_filter = {"week_of_year": week_i, "date__year": year_i}
            measurement_title = "{0}W{1}".format(year_i, week_i)
            "{0}W{1}".format(year_i, week_i)
            end_loop = start_of_week_of_year(week=week_i, year=year_i) > end_date
        else:
            raise ValueError(
                "Period {0} not valid. Only 'month' or 'week' is valid".format(period)
            )
