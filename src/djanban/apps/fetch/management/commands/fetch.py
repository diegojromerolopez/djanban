...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for member in self.members:
            if member.is_initialized:
                self.stdout.write(
                    self.style.SUCCESS(
                        "# Fetching {0} boards".format(member.external_username)
                    )
                )
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            "All boards of {0} fetched successfully".format(
                ...
                # If after two retries the exception persists, warn the administrators
                    ...
                    warn_administrators(
                        ...
                        message="We tried initializing the boards but it didn't work out. {0}".format(
                            traceback_stack
                        ),
                    )
        ...
        if fetch_ok:
            self.stdout.write(
                self.style.SUCCESS(
                    "All boards of {0} members fetched successfully {1}".format(
    ...
    def fetch_member_boards(self, member):
        ...
        while len(boards) > 0:
            ...
            if board.id not in self.fetched_boards:
                if board.is_ready():
                    try:
                        self.stdout.write(
                            self.style.SUCCESS("Board {0} is ready".format(board.name))
                        )
                        self.fetch_board(member, board)
                        self.stdout.write(
                            self.style.SUCCESS(
                                "Board {0} fetched successfully".format(board.name)
                            )
                        )
                        ...
                        traceback.print_exc()
                        error_message = "Error when fetching boards. Board {0} fetch failed. Exception {1}.".format(
                            board.name, e
                        )
                    ...
                    self.stdout.write(
                        self.style.ERROR("Board {0} is not ready".format(board.name))
                    )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        "Board {0} has already been fetched".format(board.name)
                    )
                )
        ...
        if len(failed_boards) > 0:
            self.stdout.write(
                self.style.ERROR(
                    "There are {0} boards whose fetch failed".format(len(failed_boards))
                )
            )
            for failed_board in failed_boards:
                self.stdout.write(self.style.ERROR(" - {0}".format(failed_board)))
