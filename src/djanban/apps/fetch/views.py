from django.shortcuts import render

from djanban.apps.base.decorators import member_required
from djanban.apps.fetch.management.commands.fetch import Command


# Fetch all boards, its cards and its labels
@member_required
def fetch(request):
    member = request.user.member

    # Show fetch form
    if request.method == "GET":
        replacements = {"member": member}
        return render(request, "fetch/fetch.html", replacements)

    if not member.is_initialized:
        return render(
            request,
            "fetch/fetch_error.html",
            context={
                "exception_message": f"Member {member.user.username} is not initialized"
            },
        )

    try:
        fetch_command = Command()
        fetch_command.handle(member_external_username=[member.external_username])
    except Exception as e:
        return render(
            request, "fetch/fetch_error.html", context={"exception_message": e}
        )

    return render(request, "fetch/fetch_success.html")
